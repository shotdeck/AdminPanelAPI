...
namespace ShotDeck.Keywords
{
    ...
    internal sealed class KeywordSnapshot
    {
        ...
        public static readonly KeywordSnapshot Empty = new(
            ...
            new PhraseMatcher(new HashSet<string>(StringComparer.OrdinalIgnoreCase)),
            new HashSet<string>(StringComparer.OrdinalIgnoreCase));
      ...
      "yourselves"
        ...
        public List<SearchHit> SearchWithSources(string text)
        {
            ...
            foreach (var m in matches)
            {
                ...
                hits.Add(new SearchHit(m, sources));
            }
            return hits;
        }
        public List<string> SearchForClassification(string text)
        {
            ...
            return raw.Distinct(StringComparer.OrdinalIgnoreCase).ToList();
        }
        ...
        private void EnsureWarmOrCsvThenBackgroundRefresh()
        {
            ...
            if (TryWarmFromCsv(out var warmed))
            {
                ...
                PublishStatus();
                Console.WriteLine($"[KeywordCacheService] CSV re-warm complete. Flat={warmed.FlatSet.Count}");
            }
            else
            {
        ...
        private async Task RefreshCoreAsync()
        {
            ...
            var newSources = new Dictionary<string, HashSet<string>>(StringComparer.OrdinalIgnoreCase);
            var newByCategory = new Dictionary<string, List<string>>(StringComparer.OrdinalIgnoreCase);
            void AddKeywordLocal(string keyword, string source)
            {
                ...
                newSet.Add(trimmed);
                if (!newSources.TryGetValue(trimmed, out var srcSet))
                    newSources[trimmed] = srcSet = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                srcSet.Add(source);
            }
            ...
            if (_keywordsByCategory.Count > 0)
                newByCategory = new Dictionary<string, List<string>>(_keywordsByCategory, StringComparer.OrdinalIgnoreCase);
            var newMatcher = new PhraseMatcher(newSet);
            var newSnapshot = new KeywordSnapshot(newSet, newSources, newByCategory, newMatcher, imageTags);
            SaveSnapshotToCsv(newSnapshot);
            Interlocked.Exchange(ref _snapshot, newSnapshot);
            PublishStatus();
            Console.WriteLine($"[KeywordCache] Snapshot swap: flat={_snapshot.FlatSet.Count}, sources={_snapshot.KeywordSources.Count}, cats={_snapshot.KeywordsByCategory.Count}, imageTags={_snapshot.ImageTags.Count}");
        }
        ...
        private bool TryWarmFromCsv(out KeywordSnapshot snapshot)
        {
            ...
            try
            {
                ...
                snapshot = new KeywordSnapshot(flat, sources, byCat, matcher, imageTags);
                return true;
            }
            catch (Exception ex)
            {
        ...
        private void SaveSnapshotToCsv(KeywordSnapshot snap)
        {
            ...
            {
                Directory.CreateDirectory(_csvDir);
                var flatLines = new List<string>(snap.FlatSet.Count + 1) { "keyword" };
                ...
                var srcLines = new List<string> { "keyword,source" };
                foreach (var kv in snap.KeywordSources.OrderBy(k => k.Key, StringComparer.OrdinalIgnoreCase))
                {
                    var k = Csv(kv.Key);
                    foreach (var src in kv.Value.OrderBy(v => v, StringComparer.OrdinalIgnoreCase))
                        srcLines.Add($"{k},{Csv(src)}");
                }
                File.WriteAllLines(Path.Combine(_csvDir, KeywordSnapshot.SourcesFile), srcLines, Encoding.UTF8);
                var byCatLines = new List<string> { "category,keyword" };
                foreach (var kv in snap.KeywordsByCategory.OrderBy(k => k.Key, StringComparer.OrdinalIgnoreCase))
                {
                    var cat = Csv(kv.Key);
                    foreach (var kw in (kv.Value ?? new List<string>()).OrderBy(v => v, StringComparer.OrdinalIgnoreCase))
                        byCatLines.Add($"{cat},{Csv(kw)}");
                }
